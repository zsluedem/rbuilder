...
use super::{
    ...
    handle_building_error, BlockBuildingAlgorithm, BlockBuildingAlgorithmInput,
};
...
pub fn run_preconf_builder<P>(input: LiveBuilderInput<P>, config: &PreconfBuilderConfig)
where
    P: StateProviderFactory + Clone + 'static,
{
    ...
    let preconf_list = input.ctx.preconf_list();
...
#[derive(Debug)]
pub struct PreconfBuilderContext<P> {
    provider: P,
    builder_name: String,
...
impl<P> PreconfBuilderContext<P>
where
    P: StateProviderFactory + Clone + 'static,
{
    pub fn new(
    ...
    pub fn build_block(
        ...
        preconf_list: Vec<TransactionSigned>,
    ...
    fn fill_preconf_txs(
        &mut self,
        block_building_helper: &mut BlockBuildingHelperFromProvider<P>,
        mut block_orders: PrioritizedOrderStore,
        preconf_list: Vec<TransactionSigned>,
    ) -> eyre::Result<PrioritizedOrderStore> {
...
impl<P> BlockBuildingAlgorithm<P> for PreconfBuildingAlgorithm
where
    P: StateProviderFactory + Clone + 'static,
{
    fn name(&self) -> String {
